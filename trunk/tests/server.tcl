#+
#§  \file       tests/server.tcl
#§  \brief      \$Id: server.tcl 507 2009-11-28 15:18:46Z dev1usr $
#§  
...
proc Ot_SND1 {ctx} {
  ...
  switch -exact $cmd {
    ...
    START5 {
      $ctx SlaveWorker $id --name cl-$id --srvname sv-$id --thread
    }
    STOP {
      $cl LinkDelete
    }
...
proc Ot_SND2 {ctx} {
  ...
  switch -exact $cmd {
    CREATE {
      $ctx SlaveWorker $id --name cl-$id --srvname sv-$id {*}[$ctx ReadAll]
    }
    CREATE2 {
      set c [tclmsgque MqS]
