dnl
dnl  \file       m4/ac_python_devel.m4
dnl  \brief      \$Id: ac_python_devel.m4 507 2009-11-28 15:18:46Z dev1usr $
dnl  
...
$ac_distutils_result])
	...
	if test -z "$PYTHON_CPPFLAGS"; then
		python_path=`$PYTHON -c "import distutils.sysconfig; \
           		print(distutils.sysconfig.get_python_inc(),end='');"`
	...
	if test -z "$PYTHON_SITE_PKG"; then
		python_path=`$PYTHON -c "import distutils.sysconfig; \
		        print(distutils.sysconfig.get_python_lib(0,0),end='');"`
		if test "$build_os" = "cygwin" ; then
		    python_path=`cygpath -u "$python_path"`
		fi
		PYTHON_SITE_PKG="$python_path"
	fi
